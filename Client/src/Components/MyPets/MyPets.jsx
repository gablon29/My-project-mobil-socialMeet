...
import {
  ...
  setErrorPets,
  setSuccessPets,
} from "../../Redux/ReducerPets"; //para despachar se trae la fx de redux/reducer

export default function MyPets({ navigation }) {
  const authenticatedAuth = useSelector(
    (state) => state.ReducerAuth.authenticatedAuth
  );
  const { userPets, loadingPets, errorPets, successPets } = useSelector(
    (state) => state.ReducerPets
  );
        ...
        {userPets.length ? (
          userPets.map((element, index) => (
            <View key={index} className="m-1">
              <Image
                /*source={{ uri: element.profilePic }}*/ style={{
                  width: 100,
                  height: 100,
                }}
              />
