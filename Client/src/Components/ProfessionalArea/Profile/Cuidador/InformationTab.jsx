...
const InformationTab = () => {
	...
	const professionalProfile = useSelector(state => state.ReducerProfessional.userProfessional)
	...
	useEffect(() => {
		console.log(pets);
		const initialMascotas = professionalProfile.professions.cuidador.mascotasAcuidar
		let mascotasConImagen = []
		initialMascotas.forEach((m) => {
			const mascotaACuidar = mascotas.find((ma) => ma.name === m)
			if (mascotaACuidar) {
				mascotasConImagen.push(mascotaACuidar)
			}
		})
							...
							return <View key={_idx} className="bg-celeste rounded-xl w-full p-4 shadow-2xl">
								<View className="flex flex-row items-center mb-4">
									...
									<View>
										...
										<Text className="text-white font-normal text-sm">{pet.specie} {pet.breed} {pet.age.years} años, {pet.age.months} meses</Text>
									</View>
								</View>
				...
				<View className="flex flex-col space-y-6">
					{caracteristicas.map((c) => (
						<View className="flex flex-row items-center  gap-2">
							{caracteristicasProfesional[c.caracteristica] ? check : not}
