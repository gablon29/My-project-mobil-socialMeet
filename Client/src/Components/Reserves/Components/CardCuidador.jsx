...
import StarRating from '../../../Components/ProfessionalArea/Profile/StarRating';
...
const CardCuidador = ({ professional, services, setStep, STEPS, setDisplayTotalPrice, startDate, endDate, setProductId }) => {
  ...
  return (
    <View className="flex flex-row justify-start bg-new w-full rounded-2xl h-36 p-4 my-4">
      <View className="mr-5">{professional.profilePic && <Image source={{ uri: professional.profilePic }} className="w-16 h-16 rounded-full" />}</View>
      <View>
        ...
        <TouchableOpacity
          ...
          className="flex flex-row h-8 items-center bg-black rounded-[10px] px-4 mt-2"
        >
