...
export const CreatePet6 = ({ navigation, addPet, token }) => {
   ...
   const handleAddPet = async () => {
      try {
         const agregarPet = await addPet(token, owner).then((success) => dispatch(addNewPets(success)), navigation.navigate('MyPets'));
      } catch (error) {
        ...
        {console.log('CREATE PET 6')}
         <View className='flex items-center mb-20'>
            ...
            <View className='flex items-center mt-10'>
               <Button title='Ver mi mascota' onPress={handleAddPet} colorButton='bg-naranja' colorText='text-white' ancho='w-40' alto='h-11' textSize='text-base' />
            </View>
         </View>
      </View>
   );
